...
class Applicant(BaseModel):
    ...
    resume_path: str | None = Field(
        default=None, description="Path to the resume PDF file"
    )
    resume_text: str | None = Field(
        default=None, description="Extracted text from resume"
    )
    embedding: list[float] | None = Field(
        default=None, description="Vector embedding of resume"
    )
    similarity_score: float | None = Field(
        default=None, ge=0.0, le=1.0, description="Semantic similarity to JD"
    )
...
class CandidateResponse(BaseModel):
    ...
    ai_score: int = Field(
        default=0, ge=0, le=100, description="AI-assigned score based on keywords"
    )
...
class ApplicationResponse(BaseModel):
    """Response after successful job application."""
