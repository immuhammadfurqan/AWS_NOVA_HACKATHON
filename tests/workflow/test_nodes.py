...
from app.jobs.schemas import JobInput, GeneratedJD, ApprovalStatus, RecruitmentNodeStatus
...
class TestHelperFunctions:
    ...
    def test_should_skip_jd_generation_false_by_default(self, initial_state: GraphState):
        """Test that bypass is False by default."""
        assert should_skip_jd_generation(initial_state) is False

    def test_should_skip_jd_generation_true_when_bypass_set(self, initial_state: GraphState):
    ...
    def test_filter_candidates_by_threshold(self, sample_applicants: list[Applicant]):
        ...
        shortlisted_ids = filter_candidates_by_threshold(sample_applicants, threshold=0.7)
...
class TestGenerateJDNode:
    ...
    @pytest.mark.asyncio
    async def test_raises_jd_generation_error_on_failure(self, initial_state: GraphState):
...
class TestPostJobNode:
    ...
    @pytest.mark.asyncio
    async def test_raises_not_implemented_when_disabled(self, initial_state: GraphState):
...
class TestShortlistCandidatesNode:
    ...
    async def test_shortlists_candidates_successfully(
        ...
        with patch('app.workflow.nodes.rank_candidates_by_similarity') as mock_rank:
            ...
            with patch('app.workflow.nodes.get_settings') as mock_settings:
                mock_settings.return_value = MagicMock(shortlist_similarity_threshold=0.7)
